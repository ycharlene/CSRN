...
class TimmModel(nn.Module):
    ...
    def forward_intermediates(
        ...
        """
            trunk_output[0] 最后一层的图像patch tokens结果
            trunk_output[1] 各层中间输出
        """
        return_dict = {}
        ...
        return trunk_output[0], image_features, trunk_output[2]
        # return return_dict

    def forward(self, x):
        # x = self.trunk(x)
        # x = self.head(x)
        image_feats, global_image_feat, image_attn_scores = self.forward_intermediates(x)
        
        return image_feats, global_image_feat, image_attn_scores
