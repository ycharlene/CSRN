...
class RSICD(BaseDataset):
    ...
    [{'split', str,
      ...
      'id', int}...]
    """
    dataset_dir = 'RSICD_optimal-master'

    def __init__(self, root='', verbose=True):
        super(RSICD, self).__init__()
        self.dataset_dir = op.join(root, self.dataset_dir)
        self.img_dir = op.join(self.dataset_dir, 'RSICD_images/')

        self.anno_path = op.join(self.dataset_dir, 'rsicd_qwen2_bothchat.json')
        ...
        if verbose:
            self.logger.info("=> RSICD_optimal-master Images and Captions are loaded")
