...
class RSITMD(BaseDataset):
    ...
    def __init__(self, root='', verbose=True, anno_file=None):
        super(RSITMD, self).__init__()
        self.dataset_dir = op.join(root, self.dataset_dir)
        self.img_dir = op.join(self.dataset_dir, 'imgs/')

        default_anno = 'rsitmd_Internvl3_5_QwChatv2.json'
        anno_name = anno_file or default_anno
        self.anno_path = anno_name if op.isabs(anno_name) else op.join(self.dataset_dir, anno_name)
        ...
        if training:
            ...
            for anno in annos:
                ...
                for caption in captions:
                    dataset.append((pid, image_id, img_path, caption, captions))
                    # print(dataset)
                # dataset.append((pid,pid_container,img_path,captions))
                image_id += 1
