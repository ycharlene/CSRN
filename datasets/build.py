...
from .ucm import UCM
__factory = {'CUHK-PEDES': CUHKPEDES, 'ICFG-PEDES': ICFGPEDES, 'RSTPReid': RSTPReid,'RSITMD':RSITMD,"RSICD":RSICD,"SYDNEY":SYDNEY,"UCM":UCM}


def _build_dataset(args):
    dataset_cls = __factory[args.dataset_name]
    caption_json = getattr(args, 'caption_json', '') or ''
    if caption_json and args.dataset_name == 'RSITMD':
        return dataset_cls(root=args.root_dir, anno_file=caption_json)
    return dataset_cls(root=args.root_dir)
...
def build_dataloader(args, tranforms=None):
    ...
    # # 查看训练集
    # for i, item in enumerate(dataset.train[:15]):
    #     print(f"{i}: {item}")
    ...
    if args.training:
        ...
        test_txt_loader = DataLoader(test_txt_set,
                                     ...
                                     drop_last=False, # 
                                     num_workers=num_workers)
        print('test_img_loader:',test_img_loader)
        return test_img_loader, test_txt_loader, num_classes

def build_custom_dataloader(args, tranforms=None):
    logger = logging.getLogger("IRRA.dataset")
    ...
    if args.training:
        ...
        return train_set, val_img_set, val_txt_set, num_classes
        ...
        test_txt_set = TextDataset(ds['caption_pids'],
                                   ds['captions'],
                                   text_length=args.text_length)
        if not os.path.exists('val_paths.txt'):
            with open('val_paths.txt','w') as file:
                for path in ds['img_paths']:
                    file.write(path+'\n')
        return test_img_set, test_txt_set, num_classes
